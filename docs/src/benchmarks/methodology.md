...
- **IPO** (16-slot, AND-indexed): default `HighTomb` vs the always-collides `LowTomb` (full byte-0/AND-mask overlap at any size). Sized at 100K, 1M, and 4M to show the absolute gap widening with table size.
- **IPO64** (64-slot, shift-indexed): default `LowTomb` vs the collision-prone `HighTomb` (top byte = group index → immediate collision).

Expected signal at 4M for IPO: `LowTomb` ≈ 20x slower on miss. Validates that the tag bytes stay disjoint from the group-index bits.

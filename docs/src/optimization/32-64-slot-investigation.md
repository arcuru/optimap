...
- **insert**: Tied. 16-slot `UFM` (6.2 ns), 32-slot `LowTag255ChSafe_1bit32` (6.3 ns).
- **lookup_hit**: Tied. 16-slot `HighTag128_1bitAnd` (2.1 ns), 32-slot `HighTag128_1bitAnd32` (2.1 ns).
- **lookup_miss**: 16-slot wins. `hashbrown` (1.2 ns), `LowTag128_8bit` (1.4 ns) vs best 32-slot `Splitsies32` (1.5 ns).
- **remove**: 16-slot wins slightly. `HighTag255ChSafe_EmbP2And` (12.6 ns) vs `HighTag255_1bitAnd32` (11.3 ns) vs `HighTag128_Tomb` (11.2 ns).
...
As the table grows and load factors increase, probe chains lengthen. Here, probing more slots per step _should_ help. The continuous sweep data from 100K to 2M reveals:
...
- At 1.0M N: `HighTag128_1bitAnd32` (6.4 ns) vs `HighTag128ChSafe_EmbP2And` (6.3 ns)
...
- At 1.0M N: `Splitsies32` (3.2 ns) vs `LowTag255ChSafe_EmbP2` / `Gaps` (3.0 ns)
...
- `Ufm32` vs `UFM` shows the two lines crossing repeatedly between 30 and 45 ns/op at large scale. Overall, there is no clear victor, though Tombstone designs (`HighTag128_Tomb`, 24.5 ns) significantly outperform all overflow-bit designs on this workload.
...
3. **No immediate architectural changes required.** `HighTag128_Tomb` remains the best general-purpose map, and 16-slot `Splitsies` / `UFM` remain the best tombstone-free variants.

_Plots supporting this investigation have been generated and will be preserved in `bench-results/32-64-_.png`.\*

...
IPO64 was 1.3-2.3x slower than IPO16 at all sizes. Root cause: 14 SIMD operations per probe step vs IPO16's ~4.
...
One check at entry, entire probe loop runs with best SIMD tier. No per-iteration overhead.
...
IPO64 remains 1.3-1.5x slower than IPO16 on hits. AVX-512 helped but didn't close the gap.
...
At 70% load, 64-slot groups resolve >99% of probes in a single step — same as 16-slot groups. IPO64 does more SIMD work per step (3 ops with AVX-512 vs IPO16's 2 ops) for the same number of steps. The cache-line advantage only helps when multiple steps are needed, which is rare below 95% load.
...
Further improvements (insert_no_check dispatch, iteration AVX-512) give diminishing returns. IPO64 is a research/specialty design for applications needing predictable high-load performance.

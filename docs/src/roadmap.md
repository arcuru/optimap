...
|---|------|-----------|-------|
| 14b | Port `OptiSortedMap`'s sorted-only API to `OptiMap` (first/last_key_value, pop_first/last, iter_sorted, range, range_mut, split_off, append) — and the parallel set methods to `OptiSet`. Then retire `OptiSortedMap` / `OptiSortedSet`. | Medium | Phase 2 of task 14. **Sorted API done (May 2026):** all 9 map methods + all 7 set methods ported, panicking on non-FlatBTree backends. 33 new tests. **Remaining:** retire `OptiSortedMap`/`OptiSortedSet` types (update all call sites to use `OptiMap`/`OptiSet` with `flat_btree()`/`Hint::Sorted`). |

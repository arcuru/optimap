...
|---|------|-----------|-------|
| 10 | New `Map<K, V>` facade trait (bounds: `Hash + Eq + Ord` union). Per-impl wiring delegates to whichever underlying trait is the cheapest dispatch. | Medium — boilerplate but mechanical | Not dyn-safe (generic `<Q>` methods + RPITIT block object safety). For runtime polymorphism, use `OptiMap`. |

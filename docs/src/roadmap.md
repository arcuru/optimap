...
| Item | Scope |
| --- | --- |
| CHD build-path profiling — `ChdPhf::build_with_profile` + `examples/profile_chd.rs` | Precondition for any CHD build optimization landed. **API**: `ChdBuildProfile` struct (`total`, `duplicate_check`, `bucket_assign`, `counting_sort`, `order_sort`, `displacement_search` Durations summed across all seed retries that ran, plus `seed_retries`, `bucket_count`, `max_bucket_size`, `displacement_attempts`, `max_displacement_used` counters) returned from `ChdPhf::build_with_profile(hashes, m)`; the trait-level `PerfectHashFunction::build` is now a thin wrapper. Always-on (~10 `Instant::now` per build + one u64 increment per inner-loop attempt — sub-microsecond next to a multi-hundred-ms build). Re-exported at crate root. **Example**: `cargo run --release --example profile_chd [-- 10000,100000,1000000]` prints the per-phase split + diagnostic counters. **Findings (carbon, SFC64-mixed unique u64 hashes, default λ=5)**: at **N=1M** total 868.958 ms — `displacement_search` **97.3 %** (845.713 ms), `duplicate_check` 1.9 % (16.175 ms), `counting_sort` 0.4 % (3.064 ms), `bucket_assign` 0.3 % (3.020 ms), `order_sort` 0.1 % (0.982 ms), residual ~0 %. Counters: 200 000 buckets, `seed_retries = 1`, `max_bucket_size = 18`, `displacement_attempts = 212 953 274` (1064.77 avg per bucket), `max_displacement_used = 1 322 244`. At N=100K the split is the same shape (97.8 % displacement, 1.6 % dup-check), at N=10K still 97.9 % displacement. **Implications for the queue**: (1) the displacement search itself is *the* bottleneck — every scaffolding phase (assign / counting sort / order sort) is ≤0.4 %, so optimizing them is pure noise. (2) The `max_displacement_used = 1.32 M` outlier on a 1 M-key minimal build is a long-tail signal: one or two near-saturation buckets eat a disproportionate share of attempts at minimal load. (3) **Parallel multi-seed CHD (TODO-ca38c685)** is well-targeted at the cost: 213 M attempts inside a single seed's search is embarrassingly parallel across seeds; racing N seeds → keep first success would benefit from the high per-seed variance the long-tail counter exposes. (4) Per-attempt cost is small (`slot_of` mixer + bitset test + linear scan over the tentative `Vec`) — gains there are constant-factor at best; the win is reducing *attempt count*, which means either a smarter per-bucket displacement search (PTHash's pilot table), a non-minimal `m` (slack lowers attempt count steeply), or seed-racing. (5) Duplicate check at 1.9 % is real but small — the sort-based gate at 16 ms / 1 M keys could trivially become an N-pass hash-set, but the savings are bounded by what's already a 2 % slice. **Tests**: 599/599 lib (+2 new) + 20/20 doctests pass; clippy clean. **Deferred**: parallel multi-seed using the profile to decide thread count (TODO-ca38c685, now strongly motivated); PTHash impl (TODO-57d6676f) — the pilot-table approach attacks the inner-loop attempt count directly; λ sweep (TODO-3315659b) is still useful but the profile suggests the leverage is small at minimal m. |
| `PerfectMapMultilevelBucketed` / `PerfectSetMultilevelBucketed` — 2-level bucketed PHF | Fourth ship on `src/perfect/`. Sibling to single-level `PerfectMapBucketed` that lifts the Poisson-tail λ ceiling by *accepting* level-0 overflow as routine and routing the small fraction of overflowing keys into an independent level-1 fallback. **Algorithm** (`src/perfect/bucketed/multilevel.rs`): level 0 runs a modified single-shot placement at λ₀ = 8 (default) — buckets exceeding K = 16 are marked in a 1-bit-per-bucket overflow bitset rather than aborting the seed. Their keys are collected and fed to a regular `BucketedPhf::build` at λ₁ = 4 (its independent seed family + seed-retry budget handles the small tail). At λ₀ = 8 the Poisson math gives `P(Poisson(8) > 16) ≈ 0.003` so ~0.6 % of keys reach level 1. **Lookup**: `classify(hash) -> (level, bucket)` does the level-0 bucket projection + one bit-test in the overflow bitset; on the common no-overflow path, that bit-test is the only added cost vs single-level. **Storage**: per-level tag + entry arrays (level-1 boxes empty when no overflow). At λ₀ = 8, K = 16 the level-0 tag table is `8·K/λ₀ = 16` bits/key (halved vs single-level's 32 bits/key at λ = 4) plus a `1/λ₀ = 0.125` bits/key overflow bitset; level 1 contributes ~0.2 bits/key. Slot array is ~2n at level 0 + tiny at level 1, vs single-level's 4n — entries-side memory roughly halved at scale. **API**: `PerfectMapMultilevelBucketed<K, V, S>` / `PerfectSetMultilevelBucketed<K, S>` with `from_iter_perfect` / `from_entries` / `from_keys`. `MultilevelBucketedConfig { lambda_0, lambda_1 }` exposes both knobs; `MultilevelBucketedPhf` exposes the algorithm. `OptiMap::make_perfect_multilevel_bucketed{,_with}` + `OptiSet::make_perfect_multilevel_bucketed{,_with}` on the fluent surface. Re-exported at crate root. **Bench results (bench-2026-06-04 follow-up, u64/u64, carbon)**: Construction at 1M — map 48.9 ms (20.4 Melem/s) vs single-level bucketed map 62.6 ms (16.0 Melem/s), **28 % faster** (less slot-array allocation pressure with the halved slot count); set 19.9 ms (50.2 Melem/s), matches single-level set. CHD-MPH still 786 ms (1.3 Melem/s), ~16× slower. Hit lookup at 1M — **set: 9.5 ms / 105.2 Melem/s vs single-level set 18.8 ms / 53.2 Melem/s — 2× faster**, now 2nd-fastest set at 1M behind CHD-backed PerfectSet (5.6 ms / 178 Melem/s). Map: 21.6 ms / 46.2 Melem/s vs single-level map 25.1 ms / 39.8 Melem/s (16 % faster); CHD-MPH still leads at map hit (8.1 ms / 124 Melem/s for `PerfectMap`, 5.0 ms / 200.9 Melem/s for `PerfectMapUnchecked`). Miss lookup at 1M — map 257.9 Melem/s vs single-level 285.9 (−10 %); set 266.6 Melem/s vs single-level 316.5 (−16 %). The extra bit-test on the overflow bitset hurts miss slightly. Multi-level still beats `PerfectMap` / `PerfectSet` (CHD-MPH) by ~2.5× on miss because the SIMD tag scan rejects before the entries array. At smaller N (10K, 100K) multi-level runs 5–25 % slower than single-level on every operation because cache pressure isn't biting and the extra indirection is pure overhead; the wins materialize at 1M where the halved slot array helps cache residency. **Tests**: 22 new module tests in `multilevel.rs` (build/empty/single/small/medium round-trips, classify-matches-placements, forced overflow at λ₀ = 14, map+set round-trips at 256/50 K/5 K, string keys, drop counting across both levels, iter, duplicate-hash rejection, λ-out-of-range panic, bits/key sanity). +1 doctest. 597/597 lib + 19/19 doctests pass. **Verdict**: hit-lookup at 1M improves substantially (set nearly 2× faster), construction matches or beats single-level, miss-lookup loses ~10–15 %. CHD-MPH (`PerfectMapUnchecked` / `PerfectSet`) remains the hit-lookup champion at scale — multi-level closes the gap meaningfully but does not displace CHD on hit. Keeping both single- and multi-level shipped; user picks based on workload (multi-level for hit/memory at >100K, single-level for miss-dominant or smaller N). Roadmap targets (1)/(3)/(4) met; (2) `PerfectMapUnchecked` hit-lookup match not met (closed ~50 %); (5) memory-vs-Unchecked not yet measured. **Deferred**: λ tunability sweep across {6, 8, 10, 12, 14} × N to pin the default sweet spot more rigorously; BBHash / PTHash impls if a more compact MPH is wanted; bench memory footprint directly. |
| `PerfectMapBucketed` / `PerfectSetBucketed` — bucketed PHF prototype | Third ship on `src/perfect/`. New sibling family under `src/perfect/bucketed/` — separate algorithmic class from CHD-MPH. Each key is assigned to a bucket of K=16 slots via `bucket(h, seed) mod r`; construction succeeds the first time every bucket holds at most K keys, otherwise the seed is incremented for a fresh hash family. Lookup is one bucket index + one SIMD tag scan (`_mm_load_si128` + `_mm_cmpeq_epi8` + `_mm_movemask_epi8` on a 16-byte aligned tag slab via `repr(align(16))`) + key compare on hits. **DEFAULT_LAMBDA = 4** — the original λ=12 prediction in the roadmap was wrong: at K=16, P(Poisson(12) > 16) ≈ 13 % so any sizeable r overflows every seed. The Poisson tail makes single-shot seed-retry require λ ≪ K (λ=4 ⇒ P(overflow) ≈ 1e-6 ⇒ first seed almost always succeeds at 1M). Multi-level schemes (BBHash, PTHash) recover λ near K by overflowing into a secondary structure — that's a follow-up, see Open. **Storage**: tags `Box<[BucketTags]>` (one cache line per bucket), entries `Box<[MaybeUninit<(K,V)>]>` of length `r·K`, drop walks tags to find live slots. **Tag space**: reuses `crate::hash_tag` (255 distinct values via `reduced-hash-asm`); 0x00 = empty sentinel. Tag collisions inside a bucket are resolved by the key compare on hits — they don't break the build. No `Unchecked` variant — single-byte tag collisions ~1/255 inside a bucket of 16 would silently return wrong values. **Bench results (bench-2026-06-04, u64/u64)**: Construction at 1M — bucketed map 59.5 ms vs CHD-MPH 740 ms (**12.4× faster**); bucketed set 19.6 ms (**38× faster**, matches Byte7_128_Tomb). Still 4.5× / 1.5× slower than hashbrown. Miss lookup at 1M — bucketed map 270 M/s vs CHD-MPH 61 M/s (**4.4× faster**); bucketed set 339 M/s **matches/beats hashbrown (327 M/s)** — predicted SIMD-tag-rejection win confirmed. Hit lookup at 1M — bucketed map 43.5 M/s vs CHD-MPH 132 M/s (**3× slower** at scale); set 61 M/s vs hashbrown 80 M/s. The forced λ=4 → 4n slot entries array (~64 MB at 1M for `(u64,u64)`) is cache-hostile; the roadmap's prediction that bucketed would *win* hit-lookup at large N was wrong. **API**: `PerfectMapBucketed<K, V, S=DefaultHashBuilder>` and `PerfectSetBucketed<K, S>` with `from_iter_perfect` / `from_entries` / `from_keys`. `BucketedConfig { lambda }` exposes the λ knob; `BucketedPhf` exposes the algorithm. `Placements { bucket, slot }` returned alongside the PHF for map/set construction. Re-exported at crate root. **Tests**: 24 module tests pass (empty/single/small/medium/large round trips, string keys, drop counting, custom λ, set intern pattern, duplicate-hash rejection, λ-out-of-range panic, SIMD `match_tag_16` correctness, tag-slab alignment sanity). 568/568 lib + 17/17 doctests pass. Bench suite (`benches/perfect.rs`) extended with both variants across all three groups. **Verdict**: ship as the build-cost-sensitive / miss-heavy alternative; CHD-MPH (`PerfectMapUnchecked`) remains the hit-lookup leader. Multi-level extension shipped above — partial win on hit, ~10–15 % regression on miss. |
...
### Bucketed perfect-hash family — extensions

The single-shot seed-retry prototype shipped (see Recently Completed
**bench-2026-06-04 results inline**). It confirmed the build-cost win
(12.4× vs CHD-MPH at 1M) and miss-lookup win (matches hashbrown), but
revealed that single-shot construction forces λ ≪ K (default λ = 4)
because the Poisson tail blows out the seed at K = 16, λ ≥ 8. That
forced sparsity (4n slot entries array) makes hit-lookup at large N
**lose** to CHD-MPH — opposite of what the original prediction said.
The follow-ups below are what's needed to actually beat CHD-MPH on hit
lookup *and* maintain the build/miss wins.
...
- **BBHash / PTHash impls** — λ → K (12–14) would compress the slot
  array further toward 1.3n. The 2-level prototype was the minimum
  viable shape; BBHash adds a bitvector + popcount-rank per level,
  PTHash adds a displacement table per level. Worth attempting only
  if the 2× hit-lookup gap vs CHD-MPH actually matters for a real
  workload.
- **λ tunability sweep** — `MultilevelBucketedConfig` already exposes
  `(λ₀, λ₁)`. Bench at λ₀ ∈ {6, 8, 10, 12, 14} × N ∈ {100K, 1M, 10M}
  to pin the default sweet spot empirically rather than by Poisson
  math alone.

#### Unchecked-variant exploration for bucketed

Single-byte tag collisions break the unchecked contract (one tag-twin
inside a bucket of 16 silently returns the wrong value at ~1/255
per-pair). Two paths if a bucketed `Unchecked` is wanted:

- **Wider tag (2 bytes)** — drops collision rate to 1/65k inside a
  bucket. Tag slab becomes 32 bytes per bucket; SIMD scan needs two
  cache lines or a wider load (`_mm256_*`). 16 bits/key tag overhead.
- **Drop the variant** — the tag-scanning checked map at λ=4 already
...
`BucketedConfig::with_lambda` already exposes the single-level λ knob;
`MultilevelBucketedConfig::with_lambda_0` / `with_lambda_1` expose
both multi-level knobs. What's still missing is the empirical
sweep — bench at λ ∈ {6, 8, 10, 12} × N ∈ {100K, 1M, 10M} for
single-level (probabilistic build success drops sharply past λ = 4)
and λ₀ ∈ {6, 8, 10, 12, 14} × N for multi-level (build always
succeeds, but level-1 size grows with λ₀). Pins the default sweet
spot rather than relying on Poisson math alone.
...
#### ~~Build-path profiling (precondition)~~ **SHIPPED**

`ChdPhf::build_with_profile(hashes, m)` returns a `ChdBuildProfile`
with per-phase `Duration`s + attempt counters; the existing trait
`build` is a thin wrapper that discards the profile.
`cargo run --release --example profile_chd` prints the breakdown at
N ∈ {10K, 100K, 1M} (override sizes as the first arg). Per-phase
results from the first run (carbon, default λ=5, SFC64-mixed input):

| N    | total     | dup-check | bucket-assign | counting-sort | order-sort | displacement | attempts    | avg/bucket | max d     |
| ---- | --------- | --------- | ------------- | ------------- | ---------- | ------------ | ----------- | ---------- | --------- |
| 10K  |   6.54 ms |  78 µs    |  20 µs        |  14 µs        |  20 µs     |   6.40 ms (97.9 %) |   1 844 199 |     922.10 |    57 915 |
| 100K |  79.06 ms |   1.29 ms | 212 µs        | 157 µs        |  78 µs     |  77.32 ms (97.8 %) |  19 940 154 |     997.01 |    90 727 |
...
**Expected impact**: 4–8× wall-clock build speedup at large N — the
profile data confirms this as the queue's highest-value perf item.
...
default dep set minimal. The 869 ms / 1M number measured by the
profile run above puts this in the "real user-visible latency"
regime, and the profile's `max_displacement_used = 1.32 M` on a
1 M-key minimal build shows the long-tail variance — racing N seeds
...
| Item | Notes |
| --- | --- |
| Narrow `Lookup<K, V>` trait | `get` + `contains_key` + `iter` + `len` only. Implemented by both `PerfectMap*` and the mutable maps (hashbrown, `Tomb`, etc.). Specifically **not** a `ReadMap` super-trait that `Map` extends — collection-level read/write trait splits have essentially no precedent in Rust: `std` exposes no `Map` trait; `hashbrown`/`indexmap`/`dashmap`/`im` are all concrete types or parallel hierarchies; `AsyncRead`/`AsyncWrite` are byte streams, not collections; `Index`/`IndexMut`/`Borrow`/`BorrowMut` are single-method primitives. The honest shape is a peer trait purpose-built for lookup-only consumers, not a generalization of `Map`. Returning `Err` from mutator methods on a unified `Map` impl was also considered and rejected — most `Map` methods don't return `Result` today (`insert -> Option<V>`, `clear() -> ()`, `iter_mut`, `drain`), so it would require breaking every existing impl, and the pattern itself is Java-style runtime refusal rather than Rust-style type-system separation. Ship when the criterion bench above wants one generic body across map types; until then, the bench harness can use macros over a type list (the same pattern `for_each_design!` already uses for sweep benches). |

...
|---|------|-----------|-------|
| 9 | Extend `SortedMap` with full CRUD (`insert`/`get`/`remove`/`contains_key`/...) bounded `Q: Ord`. Implement on `FlatBTree` and `std::BTreeMap`. | Low — additive, no churn | Unblocks task 12. |
...
| 12 | Switch `benches/sweep_btree.rs` from its private `BTreeBench` trait to `M: SortedMap<u64, u64>`. | Low — small refactor | Depends on task 9. |

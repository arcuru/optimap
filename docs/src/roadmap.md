...
| Item | Difficulty | Notes |
|------|-----------|-------|
| Tree-surgery `append` (graft instead of drain+rebuild) | Medium | `FlatBTree::append(&mut other)` is currently O(n+m) drain → bulk_load. When the two trees' key ranges are disjoint and adjacent (the common case of "split then re-merge"), they could be grafted: link `self`'s last leaf next-pointer to `other`'s first leaf, splice `other`'s spine into the appropriate slot of `self`'s root path, propagate parent pointers, rebalance only along the seam. Expected O(log(n+m) + seam_rebalance) vs O(n+m). Trickier than `split_off` because the heights may differ (need to descend into the taller tree to find the insert point), and overlapping ranges still need fallback to drain. Bench reference: at 100K we already tie std (~5% faster); at 1M the drain dominates. |
| Estimator bias correction | Low-Medium | `estimate_right_fraction` over-counts the right side by ~+0.10 (uniform-subtree-weighting; the rightmost top-level subtree is usually partial). Currently absorbed by raising the dispatcher cutoff from 0.5 to 0.6. Could be fixed structurally by tracking the actual entry count of the rightmost top-level subtree (one extra metadata field on `RawBTree`, updated on insert/remove). Cleaner but only matters near the symmetric pivot — current routing is already optimal at the asymmetric pivots that matter most. Defer until a workload demonstrates this is the bottleneck. |
| Investigate std::BTreeMap split_off win at 1M-p050 | Low | At 1M entries, p050 surgical split: std 6.6ms vs ours 10.4ms (~1.6× behind). At p001/p010 surgical_left ties or beats std. Hypothesis: std's structural split avoids any deep-copy whatsoever (it relinks owned `Box<Node>` chains across the split), while we still pay node-byte-copy + arena-alloc for the smaller side. Profile and confirm; possible mitigation is a "subtree-grafting" path that moves entire subtrees as `Arena` slot ranges without per-node copy. |

...
macro_rules! for_each_design {
    ($config:expr, $callback:ident $(, $arg:expr)*) => {
        ...
        for_each_design!(@run $config, $callback, Byte0_128_EmbMap<u64,u64>, "Byte0_128_Emb" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte0_128_EmbP2Map<u64,u64>, "Byte0_128_EmbP2" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128Ch_EmbAndMap<u64,u64>, "Byte7_128Ch_EmbAnd" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128Ch_EmbP2AndMap<u64,u64>, "Byte7_128Ch_EmbP2And" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_255Ch_EmbAndMap<u64,u64>, "Byte7_255Ch_EmbAnd" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_255Ch_EmbP2AndMap<u64,u64>, "Byte7_255Ch_EmbP2And" $(, $arg)*);
        ...
        for_each_design!(@run $config, $callback, Byte7_128_1bitAnd32Map<u64,u64>, "Byte7_128_1bitAnd32" $(, $arg)*);
        ...
        for_each_design!(@run $config, $callback, Byte1_Emb32Map<u64,u64>, "Byte1_Emb32" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte1_EmbP232Map<u64,u64>, "Byte1_EmbP232" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte0_128_Emb32Map<u64,u64>, "Byte0_128_Emb32" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte0_128_EmbP232Map<u64,u64>, "Byte0_128_EmbP232" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128Ch_EmbAnd32Map<u64,u64>, "Byte7_128Ch_EmbAnd32" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128Ch_EmbP2And32Map<u64,u64>, "Byte7_128Ch_EmbP2And32" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_255Ch_EmbAnd32Map<u64,u64>, "Byte7_255Ch_EmbAnd32" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_255Ch_EmbP2And32Map<u64,u64>, "Byte7_255Ch_EmbP2And32" $(, $arg)*);
        ...
        for_each_design!(@run $config, $callback, Byte7_128_1bitAnd64Map<u64,u64>, "Byte7_128_1bitAnd64" $(, $arg)*);
        ...
        for_each_design!(@run $config, $callback, Byte1_Emb64Map<u64,u64>, "Byte1_Emb64" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte1_EmbP264Map<u64,u64>, "Byte1_EmbP264" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte0_128_Emb64Map<u64,u64>, "Byte0_128_Emb64" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte0_128_EmbP264Map<u64,u64>, "Byte0_128_EmbP264" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128Ch_EmbAnd64Map<u64,u64>, "Byte7_128Ch_EmbAnd64" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128Ch_EmbP2And64Map<u64,u64>, "Byte7_128Ch_EmbP2And64" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_255Ch_EmbAnd64Map<u64,u64>, "Byte7_255Ch_EmbAnd64" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_255Ch_EmbP2And64Map<u64,u64>, "Byte7_255Ch_EmbP2And64" $(, $arg)*);
        // Tombstone variant
        for_each_design!(@run $config, $callback, Byte2_254_TombMap<u64,u64>, "Byte2_254_Tomb" $(, $arg)*);
        for_each_design!(@run $config, $callback, Byte7_128_TombMap<u64,u64>, "Byte7_128_Tomb" $(, $arg)*);
        // IPO64 tombstone variants
        for_each_design!(@run $config, $callback, Byte7_254_Tomb64Map<u64,u64>, "Byte7_254_Tomb64" $(, $arg)*);
        // SoA variants
        ...
        for_each_design!(@run $config, $callback, optimap::soa::SoaByte7_128_8bitAnd<u64,u64>, "SoaByte7_128_8bitAnd" $(, $arg)*);
        for_each_design!(@run $config, $callback, optimap::soa::SoaByte7_255_8bitAnd<u64,u64>, "SoaByte7_255_8bitAnd" $(, $arg)*);
        // SoA tombstone variants
        for_each_design!(@run $config, $callback, optimap::soa::SoaIpo<u64,u64>, "SoaIpo" $(, $arg)*);
        for_each_design!(@run $config, $callback, optimap::soa::SoaByte7_128_Tomb<u64,u64>, "SoaByte7_128_Tomb" $(, $arg)*);
    };

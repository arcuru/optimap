...
impl<K, V> RawTable<K, V> {
    ...
    /// Stride for bucket array: 16 slots per group (15 usable + 1 gap).
    /// This makes bucket_ptr a power-of-2 shift instead of multiply-by-15.
    const BUCKET_STRIDE: usize = 16;

    /// Compute the bucket offset within a single allocation.
    /// Layout: [metadata: num_groups*16] [padding] [buckets: num_groups*16*sizeof(K,V)]
    #[inline(always)]
    ...
    fn combined_layout(num_groups: usize) -> Layout {
        ...
        // Allocate 16 slots per group (15 usable + 1 gap)
        let total_buckets = num_groups * Self::BUCKET_STRIDE;
    ...
    /// Pointer to bucket slot. Uses power-of-2 stride (16 slots per group).
    #[inline(always)]
    pub(crate) unsafe fn bucket_ptr(&self, gi: usize, si: usize) -> *mut (K, V) {
        let bucket_size = std::mem::size_of::<(K, V)>();
        let idx = (gi << 4) | si; // 16-slot stride, same as Splitsies
    ...
    pub(crate) fn insert_no_check(&mut self, h: u64, key: K, value: V) -> (usize, usize) {
        ...
        loop {
            ...
            let ofw_bit = overflow_bit(h);
            unsafe {
                Group::set_overflow_bit(meta, ofw_bit);
            }
        ...
        if full_mask != 0 {
            ...
            while mask != 0 {
                if mask & 1 != 0 {
                    ...
                    unsafe {
                        Group::set_overflow_bit(set_meta, ofw_bit);
                    }
                }
                mask >>= 1;
                set_probe += 1;

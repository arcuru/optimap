...
pub struct IPO64<K, V, S = DefaultHashBuilder> {
    table: RawTable<K, V>,
    hash_builder: S,
}
...
impl<K, V> IPO64<K, V, DefaultHashBuilder> {
    pub fn new() -> Self {
    ...
    S: BuildHasher,
{
    ...
    pub fn insert(&mut self, key: K, value: V) -> Option<V> {
        use super::raw::group::{Group, reduced_hash, EMPTY};
...
pub struct IntoIter<K, V> {
    table: RawTable<K, V>,
    group: usize,
    current_mask: super::raw::group::BitMask64,
}
        ...
        loop {
            ...
            self.current_mask = unsafe {
                Group::match_occupied(
                    self.table.metadata.add(self.group << 6)
                )
            };
        }
    }
...
impl<K, V> Default for IPO64<K, V, DefaultHashBuilder> {
    fn default() -> Self { Self::new() }
}

impl<K, V, S> IntoIterator for IPO64<K, V, S> {
    type Item = (K, V);
    type IntoIter = IntoIter<K, V>;

    fn into_iter(self) -> IntoIter<K, V> {
        use super::raw::group::{Group, BitMask64};
...
impl<'a, K, V, S> IntoIterator for &'a IPO64<K, V, S>
...
impl<K, V, S> FromIterator<(K, V)> for IPO64<K, V, S>
...
impl<K, V, S> Extend<(K, V)> for IPO64<K, V, S>
where K: Hash + Eq, S: BuildHasher,
{
...
impl<K, V, S, Q> Index<&Q> for IPO64<K, V, S>
...
impl<K, V, S> fmt::Debug for IPO64<K, V, S>
...
impl<K, V, S> Clone for IPO64<K, V, S>
...
impl<K, V, S> PartialEq for IPO64<K, V, S>
...
mod tests {
    ...
    #[test]
    fn with_capacity() {
        let map: IPO64<i32, i32> = IPO64::with_capacity(100);
...
impl<K, V, S> crate::traits::Map<K, V> for IPO64<K, V, S>
where
    K: std::hash::Hash + Eq,

...
use crate::raw::hash;
use crate::raw::tag_strategy::{LowTomb, TombstoneTag};
...
///
/// Default tag is `LowTomb` (bits 0-7) — IPO64 uses shift indexing
...
pub struct RawTable<K, V, T: TombstoneTag = LowTomb> {
...
impl<K, V, T: TombstoneTag> RawTable<K, V, T> {
        ...
        F: Fn(&K) -> bool,
    {
        ...
        Some(unsafe { self.bucket_ptr(gi, si) })
    }
...
pub struct SlotIter<'a, K, V, T: TombstoneTag = LowTomb> {
...
pub struct IntoIter<K, V, T: TombstoneTag = LowTomb> {
...
impl<K, V, T: TombstoneTag> RawTableApi<K, V> for RawTable<K, V, T> {
    ...
    unsafe fn value_ptr(&self, gi: usize, si: usize) -> *mut V {
        unsafe { &raw mut (*self.bucket_ptr(gi, si)).1 }
    }

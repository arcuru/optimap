# Three-Way Benchmark Results (2026-04-02)

All benchmarks at 70% load factor unless noted. Pre-warmed tables for
throughput tests. Comparing: ours (15-slot), split16/Splitsies (16-slot),
hashbrown 0.15. All use foldhash.

Ratios: <1.0 means faster than hashbrown, >1.0 means hashbrown is faster.

## Throughput (pre-warmed, 70% load)

Medium = ~10,752 entries in 15,360 slots (1,024 groups, ours) / 16,384 slots (split16).
Large = ~86,016 entries in 122,880 slots (8,192 groups, ours) / 131,072 slots (split16).

| Operation | Size | ours | split16 | hashbrown | ours vs hb | split16 vs hb |
|-----------|------|-----:|--------:|----------:|:----------:|:-------------:|
...
| **insert 128B** | large | 955 µs | 989 µs | 973 µs | 0.98x | 1.02x |
...
| insert existing | large | 299 µs | 309 µs | 251 µs | 1.19x | 1.23x |
...
| grow_from_empty | 1M | 36.2 ms | 36.6 ms | 33.9 ms | 1.07x | 1.08x |
...
| Distribution | Op | ours | split16 | hashbrown | split16 vs hb |
|-------------|-----|-----:|--------:|----------:|:-------------:|
| random | hit | 246 µs | 238 µs | 213 µs | 1.12x |
| sequential | hit | 246 µs | 244 µs | 208 µs | 1.17x |
| byteswapped | hit | 246 µs | 249 µs | 202 µs | 1.23x |
| random | miss | 143 µs | **125 µs** | 127 µs | **0.98x** |
| **sequential** | **miss** | 121 µs | **106 µs** | 143 µs | **0.74x** |
| byteswapped | miss | 137 µs | 126 µs | 112 µs | 1.13x |
| random | insert | 309 µs | 312 µs | 323 µs | **0.97x** |
| sequential | insert | 306 µs | **304 µs** | 321 µs | **0.95x** |

## Value Sizes (medium table, 70% load)

| Value Size | Op | ours | split16 | hashbrown | split16 vs hb |
|-----------|-----|-----:|--------:|----------:|:-------------:|
| 64B | insert | 45.7 µs | **42.7 µs** | 43.2 µs | **0.99x** |
| 64B | hit | 24.8 µs | **21.8 µs** | 20.2 µs | 1.08x |
| 128B | insert | 81.8 µs | 93.2 µs | 62.8 µs | **1.48x** ⚠️ |
| 128B | hit | 29.7 µs | **28.3 µs** | 25.4 µs | 1.11x |
| 256B | insert | 166 µs | 165 µs | 100 µs | **1.65x** ⚠️ |
| 256B | hit | 30.1 µs | **26.5 µs** | 25.7 µs | 1.03x |

⚠️ Large-value insert regression for split16 at 128B+ needs investigation.

## String Key Sizes (medium table, lookup hit)

| Key Length | ours | split16 | hashbrown | split16 vs hb |
|-----------|-----:|--------:|----------:|:-------------:|
| 7b | 56.9 µs | 60.3 µs | 56.5 µs | 1.07x |
| 8b | — | — | — | — |
| 13b | — | — | — | — |
| 24b | — | — | — | — |
| 100b | — | — | — | — |

(String key benchmarks had partial data; 7b shown as representative.)

## Mixed Workloads

| Workload | ours | split16 | hashbrown | ours vs hb | split16 vs hb |
|----------|-----:|--------:|----------:|:----------:|:-------------:|
...
| counting 100% | 244 ms | 245 ms | 181 ms | 1.35x | 1.35x |
| post-del medium | 17.5 µs | **15.2 µs** | 12.8 µs | 1.37x | **1.19x** |
| post-del large | 175 µs | **166 µs** | 143 µs | 1.22x | **1.16x** |

## Miss Ratio Sweep (large table)

| Miss % | ours | split16 | hashbrown | split16 vs hb |
|-------:|-----:|--------:|----------:|:-------------:|
| 0% | 226 µs | 230 µs | 184 µs | 1.25x |
| 25% | 203 µs | 200 µs | 173 µs | 1.16x |
| 50% | 179 µs | **170 µs** | 152 µs | **1.12x** |
| 75% | 156 µs | **148 µs** | 137 µs | **1.08x** |
| 100% | 136 µs | **127 µs** | 123 µs | **1.03x** |

## Summary: Splitsies vs hashbrown

### Splitsies wins
- **Insert** (u64 values): 0.93-0.97x
- **Remove**: 0.64-0.84x (tombstone-free deletion)
- **Churn**: 0.62-0.86x (our biggest advantage)
- **Read-heavy workload**: 0.99x (tied!)
- **Sequential miss**: 0.74x (overflow design shines)
- **Random miss**: 0.98x (nearly tied)
- **from_iter**: 0.62-0.70x
- **with_capacity**: 0.82-0.96x
- **Clone 100K**: 0.97x

### hashbrown wins
- **Lookup hit**: 1.11-1.23x (structural per-probe overhead)
- **Entry API**: 1.66x (hit overhead + enum construction)
- **Large value insert (128B+)**: 1.48-1.65x ⚠️ needs investigation
- **Write-heavy**: 1.14x
- **Counting/aggregation**: 1.28-1.35x
- **Miss at 0% miss**: 1.25x (all-hit = lookup hit performance)

### Splitsies improvements over 15-slot
